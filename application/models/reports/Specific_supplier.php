...
class Specific_supplier extends Report
{
	...
	public function getInputData()
	{
		...
		if ($this->settings['display'] == 'tabular')
		{
			...
			$input_params[] = array('view' => 'dropdown','dropdown_label' =>lang('reports_receiving_type'),'dropdown_name' => 'receiving_type','dropdown_options' =>array('all' => lang('reports_all'), 'receiving' => lang('reports_receiving'), 'returns' => lang('reports_returns')),'dropdown_selected_value' => 'all');
	...
	public function getOutputData()
	{
		...
		{		
			...
			$summary_data_row[] = array('data'=>anchor('receivings/receipt/'.$row['receiving_id'], '<i class="ion-printer"></i>', array('target' => '_blank')).' '.anchor('receivings/edit/'.$row['receiving_id'], '<i class="ion-document-text"></i>', array('target' => '_blank')).' '.anchor('receivings/edit/'.$row['receiving_id'], lang('common_edit').' '.$row['receiving_id'], array('target' => '_blank')), 'align'=>'left','detail_id' => $row['receiving_id']);
	...
	public function getData()
	{
		...
		$this->db->select('locations.name as location_name, receiving_id, receiving_time, date(receiving_time) as receiving_date, total_quantity_purchased as items_purchased, CONCAT(employee.first_name," ",employee.last_name) as employee_name, subtotal, total, tax, profit, payment_type, comment', false);
		...
		$this->db->where(' receiving_time BETWEEN '. $this->db->escape($this->params['start_date']). ' and '. $this->db->escape($this->params['end_date']).' ');
		$this->db->where('receivings.suspended = 0');
		
		/*if (isset($this->settings['force_suspended']) && $this->settings['force_suspended'])
		{
			$where .=' and (receivings.suspended != 0 or (was_layaway = 1 or was_estimate = 1))';				
		}
		elseif ($this->config->item('hide_layaways_sales_in_reports'))
		{
			$where .=' and receivings.suspended = 0';
		}
		else
		{
			$where .=' and receivings.suspended < 2';					
		}
		$this->db->where($where);
		echo "<pre>";
		print_r ($where);
		...
		$this->db->order_by('receivings.receiving_time', ($this->config->item('report_sort_order')) ? $this->config->item('report_sort_order') : 'asc');
		...
		if (isset($this->params['export_excel']) && $this->params['export_excel'] == 1)
		{
			...
			{
				$sale_ids[] = $sale_row['receiving_id'];
			}
	...
	/*public function getTotalRows_copia()
	{		
		$location_ids = self::get_selected_location_ids();
	...
	public function get_total_a_cuenta()
	{
		...
		$this->db->where('supplier_id', $this->params['supplier_id']);
		$this->db->where('total_quantity_purchased <> 0');
		$this->db->where('receivings_payments.payment_type','Línea de crédito');
		$this->db->or_where('receivings_payments.payment_type', 'Store Account');
		$this->db->where(' receiving_time BETWEEN '. $this->db->escape($this->params['start_date']). ' and '. $this->db->escape($this->params['end_date']).' ');
		$this->db->where('deleted', 0);
		$this->db->where('receivings.suspended = 0');
		/*
