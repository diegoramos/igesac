...
class Summary_categories extends Report
{
	...
	function getOutputData()
	{
		...
		{				
			...
			foreach($report_data as $row)
			{
				...
				$data_row[] = array('data'=>floatval($row['total_items_in_inventory']).($do_compare && $row_compare ? ' / <span class="compare '.($row_compare['total_items_in_inventory'] >= $row['total_items_in_inventory'] ? ($row['total_items_in_inventory'] == $row_compare['total_items_in_inventory'] ?  '' : 'compare_better') : 'compare_worse').'">'.floatval($row_compare['total_items_in_inventory']) .'</span>':''), 'align' => 'right');
				$data_row[] = array('data'=>to_currency($row['inventory_total']).($do_compare && $row_compare ? ' / <span class="compare '.($row_compare['inventory_total'] >= $row['inventory_total'] ? ($row['inventory_total'] == $row_compare['inventory_total'] ?  '' : 'compare_better') : 'compare_worse').'">'.to_currency($row_compare['inventory_total']) .'</span>':''), 'align' => 'right');
				$data_row[] = array('data'=>to_currency($row['inventory_sale_total']).($do_compare && $row_compare ? ' / <span class="compare '.($row_compare['inventory_sale_total'] >= $row['inventory_sale_total'] ? ($row['inventory_sale_total'] == $row_compare['inventory_sale_total'] ?  '' : 'compare_better') : 'compare_worse').'">'.to_currency($row_compare['inventory_sale_total']) .'</span>':''), 'align' => 'right');
	...
	public function getDataColumns()
	{
		...
		$columns[] = array('data'=>lang('reports_ending_quantity'), 'align'=> 'right');
	...
	{	
		...
		$category_inventory = $this->getInventoryByCategory();
		return $this->merge_items_and_item_kits_with_category_inventory($items_and_kits,$category_inventory);
	}
	...
	private function merge_items_and_item_kits_with_category_inventory($items_and_kits,$category_inventory)
	{
		foreach(array_keys($items_and_kits) as $category_id)
		{
			$items_and_kits[$category_id]['total_items_in_inventory'] = isset($category_inventory[$category_id]['total_items_in_inventory']) ? $category_inventory[$category_id]['total_items_in_inventory']: null;
			$items_and_kits[$category_id]['inventory_total'] = isset($category_inventory[$category_id]['inventory_total']) ? $category_inventory[$category_id]['inventory_total'] : NULL;
			$items_and_kits[$category_id]['inventory_sale_total'] = isset($category_inventory[$category_id]['inventory_sale_total']) ? $category_inventory[$category_id]['inventory_sale_total'] : NULL;
			$items_and_kits[$category_id]['weighted_cost'] = isset($category_inventory[$category_id]['weighted_cost']) ? $category_inventory[$category_id]['weighted_cost'] : NULL;
		}
	...
	{
		$date = isset($this->params['end_date']) ? $this->params['end_date'] : FALSE;
		...
		$this->db->select('category_id,'.$full_sum_query.' as total_items_in_inventory, SUM(IFNULL('.$this->db->dbprefix('location_items').'.cost_price, '.$this->db->dbprefix('items').'.cost_price) * ('.$quantity_query.')) as inventory_total,
