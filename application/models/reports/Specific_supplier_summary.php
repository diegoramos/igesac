...
class Specific_supplier_summary extends Report
{
	...
	public function getOutputData()
	{
		...
		{			
			...
			{
                            $tableName = $this->db->dbprefix('suppliers');
                            $strSQLsup=$this->db->query("SELECT company_name FROM ".$tableName." WHERE person_id=".$drow['supplier_id']."");
                            $rowsupname=$strSQLsup->row();
                            
                            
                            
				$details_data_row = array();
                                $details_data_row[] = array('data'=>date(get_date_format(),strtotime($drow['sale_date'])), 'align'=> 'left');
                                $details_data_row[] = array('data'=>$rowsupname->company_name, 'align'=> 'left');
