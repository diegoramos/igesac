...
class Summary_customers extends Report
{
	...
	{
		$this->db->select('COUNT(*) as count,customers.current_sales_for_discount, customer_id, CONCAT(first_name, " ",last_name) as customer, customers.person_id as person_id, people.phone_number, sum(subtotal) as subtotal, sum(total) as total, sum(tax) as tax,sum(profit) as profit, sum(total_quantity_purchased) as total_quantity_purchased', false);
		$this->db->from('sales');
	...
	{
		$this->db->select('"" as points_used,"" as points_gained,COUNT(*) as count,'.$this->db->escape(lang('reports_no_customer')).' as customer, "-" as person_id, "-" as phone_number, sum(subtotal) as subtotal, sum(total) as total, sum(tax) as tax,sum(profit) as profit,sum(total_quantity_purchased) as total_quantity_purchased', false);
		$this->db->from('sales');
		if ($this->params['sale_type'] == 'sales')
		{
			$this->db->where('total_quantity_purchased > 0');
