...
{		
	...
	public static function get_instance_from_sale_id($sale_id,$cart_id = NULL,$is_editing_previous = FALSE)
	{
		...
		foreach($CI->Sale->get_sale_items($sale_id)->result() as $row)
		{
			...
			$item_props['image_id'] = $cur_item_info->image_id;
		...
		$cart->type_comprobante = $sale_info['type_comprobante'];
	...
	function determine_new_prices_for_tier_change()
	{
		...
		foreach ($items as $line=>$item )
		{
			if (get_class($item) == 'PHPPOSCartItemSale')
			{
				...
				if((isset($item_variation_info) && $price == $item_variation_info->unit_price) || $price==$item_info->unit_price || $price == $item_location_info->unit_price || (($price == $previous_price) && (($price !=0 && $previous_price!=0) || $previous_tier_id)))
				{	
			...
			elseif (get_class($item) == 'PHPPOSCartItemKitSale')
			{
				...
				if($price==$item_kit_info->unit_price || $price == $item_kit_location_info->unit_price || (($price == $previous_price) && (($price !=0 && $previous_price!=0) || $previous_tier_id)))
				{
		...
		foreach($this->get_items() as $item)
		{
			if ($item->tax_included == 1) {
				...
				else
				{
					...
					{
						$total_discount+=(($item->regular_price-$item->unit_price)*$item->discount/100)*$item->quantity;				
					}
				}
			} else {
				if ($item->item_id == $CI->Item->get_item_id_for_flat_discount_item())
