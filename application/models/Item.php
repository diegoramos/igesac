...
class Item extends CI_Model
{
	...
	*/
	...
	function get_displayable_columns()
	{
		$return  = array(
			...
			'unity' => 											array('sort_column' => 'unity', 'label' => lang('common_unit')),
	...
	*/
	function get_all($deleted=0,$limit=10000, $offset=0,$col='item_id',$order='desc')
	{
		...
		if ($attribute_count > 0 && !$this->config->item('speed_up_search_queries'))
		{
			...
			$phppos_unit = $this->db->dbprefix('unit');
			...
			$query = "SELECT main_query.*,tags_query.tags as tags,image_query.image_id as image_id,unity
			FROM (
				...
				`$phppos_categories`.`name` as `category`, `$phppos_unit`.`name` as `unity`,
	        ...
	        LEFT JOIN `$phppos_location_item_variations` 
	          ON `$phppos_location_item_variations`.`item_variation_id` = `$phppos_item_variations`.`id` and `$phppos_location_item_variations`.`location_id` = $current_location
					...
					LEFT JOIN `$phppos_unit` 
						ON `$phppos_unit`.`unit_id` = $phppos_items.unit_id
		...
		else
		{
			...
			if (!$this->config->item('speed_up_search_queries'))
			{
				$this->db->select($this->db->escape(lang('common_inv')).' as inventory, suppliers.company_name as supplier_company_name, location_items.location as location,unit.`name` as `unity`, items.*, item_images.image_id as image_id, GROUP_CONCAT(DISTINCT '.$this->db->dbprefix('tags').'.name) as tags, categories.id as category_id,categories.name as category,
			...
			{
				$this->db->select($this->db->escape(lang('common_inv')).' as inventory, suppliers.company_name as supplier_company_name, location_items.location as location,unit.`name` as `unity`, items.*, 0 as image_id, "" as tags, categories.id as category_id,categories.name as category,
				location_items.quantity as quantity,0 as variation_count, 0 as has_variations, 
				location_items.reorder_level as location_reorder_level,
				location_items.cost_price as location_cost_price,
				location_items.unit_price as location_unit_price,
			...
			$this->db->join('unit', 'unit.unit_id = items.unit_id','left');
	...
	*/
	function get_info($item_id, $can_cache = TRUE)
	{
		...
		$this->db->select('items.*,unit.name as unity');
		$this->db->from('items');
		$this->db->join('unit', 'items.unit_id = unit.unit_id','left');
		$this->db->where('item_id',$item_id);
	...
	*/
	...
	{	
		/// cambio unidad obligatorio unidad de medida
		$this->db->select('items.*,unit.name as unity');
		$this->db->from('items');
					...
					else
					{
						...
						else
						{
							...
							foreach ($search_terms_array as $x)
							{
								...
								$this->db->dbprefix('items').".name LIKE '".(!$this->config->item('speed_up_search_queries') ? '%' : '').$this->db->escape_str($x)."%' ESCAPE '!'";
...
=======
						...
						ON `$phppos_unit`.`unit_id` = $phppos_items.unit_id
					$search_main_query 
>>>>>>> 2af31221e9700c707b674351963b306ccf7a09e7
		...
		else
		{
			...
			if (!$this->config->item('speed_up_search_queries'))
			{
				$this->db->select('"'.lang('common_inv').'" as inventory, suppliers.company_name as supplier_company_name,location_items.location as location, item_images.image_id as image_id, GROUP_CONCAT(DISTINCT '.$this->db->dbprefix('tags').'.name) as tags,items.*,categories.id as category_id,categories.name as category,unit.`name` as `unity`,
				location_items.quantity as quantity,0 as variation_count, 0 as has_variations, 
				location_items.reorder_level as location_reorder_level,
				location_items.cost_price as location_cost_price,
				location_items.unit_price as location_unit_price,
			...
			{
				$this->db->select('"'.lang('common_inv').'" as inventory, suppliers.company_name as supplier_company_name,location_items.location as location, 0 as image_id, "" as tags,items.*,categories.id as category_id,categories.name as category,unit.`name` as `unity`,
				location_items.quantity as quantity,0 as variation_count, 0 as has_variations, 
				location_items.reorder_level as location_reorder_level,
				location_items.cost_price as location_cost_price,
				location_items.unit_price as location_unit_price,
	...
	{
		return array('item_number','name','unity','category','cost_price','unit_price','quantity');
	...
	*/
	...
	function is_item_in_quickbooks($accounting_id)
	{
		$this->db->from('items');	
		$this->db->where('items.accounting_id',$accounting_id);
		$this->db->where('items.deleted',0);
		$query = $this->db->get();
	...
	function get_info_by_accounting_id($accounting_id)
	{
		$this->db->from('items');	
		$this->db->where('items.accounting_id',$accounting_id);
		$this->db->where('items.deleted',0);
		...
		if ($query->num_rows()==1)
		{
			return $this->get_info($query->row()->item_id);
		}
		
		return $this->get_info(0);
	}
	
	function get_items_not_in_quickbooks_or_modified_since_last_sync()
	{
		$last_qb_sync_date  = $this->config->item('last_qb_sync_date') ? $this->config->item('last_qb_sync_date') : date('Y-m-d H:i:s');
		$items=$this->db->dbprefix('items');
		$data=$this->db->query("SELECT * 
						FROM ".$items."
						WHERE (accounting_id IS NULL) or (accounting_id IS NOT NULL and last_modified > '$last_qb_sync_date')");		
						
		return $data;
	}
	
	function link_item_to_quickbooks($item_id,$accounting_id)
	{
		if ($item_id && $accounting_id)
		{
			$this->db->where('item_id',$item_id);
			$this->db->update('items',array('accounting_id' => $accounting_id));
		}
	}

	// Custom Functions Start
	function get_item_with_accounting_id($accounting_id)
	{
		$this->db->from('items');	
		$this->db->where('items.deleted',0);
		$this->db->where('items.accounting_id',$accounting_id);
	...
	// update the flag from 1 to 0 of old entries for a item, whenever new inventory change has been done. So that we can get the final quantity only and ignore rest. It will help in saving extra api calls
	function update_inventory_trans($item_id) {
		$this->db->where('trans_items', $item_id);
		$this->db->where('inventory_sync_needed', INVENTORY_SYNC_NEEDED);
		$this->db->update('inventory',array('inventory_sync_needed' => 0));
	}

	// function to get the array of (sum of qty adjustments, item name, item id and accounting id) for the items that are pending for the sync
	function get_inventory_items_to_sync() {
		$last_qb_sync_date  = $this->config->item('last_qb_sync_date') ? $this->config->item('last_qb_sync_date') : date('Y-m-d H:i:s');
		$this->db->select('inventory.trans_items, sum(trans_inventory) as trans_inventory, items.name, items.accounting_id');
		$this->db->from('inventory');
		$this->db->join('items', 'items.item_id = inventory.trans_items','left');
		$this->db->where('inventory.inventory_sync_needed',1);
		$this->db->where('inventory.trans_date >',$last_qb_sync_date);
