...
class Employee extends Person
{
	 ...
	 $this->db->select('employees_locations.location_id,employees.*,people.*,people.person_id as pid');
 	 ...
 	 $this->db->select('employees_locations.location_id,employees.*,people.*,people.person_id as pid');
		...
		elseif ($search)
		{
			...
			$this->db->where("$custom_fields LIKE '".$this->db->escape_like_str($search)."%' and deleted=$deleted");		
		}
		else
		{
			$this->db->where('deleted',$deleted);
	...
	*/
	function login($username, $password)
	{
		...
		if ($query->num_rows() ==1)
		{
			...
			$this->session->set_userdata('isadmin', $row->isadmin);//Solo para filtro de ventas suspendidas
			return true;
		}
	...
	*/
	...
	function get_employees_not_in_quickbooks_or_modified_since_last_sync()
	{
	...
	function link_employee_to_quickbooks($person_id,$accounting_id)
	{
		if ($person_id && $accounting_id)
		{
			$this->db->where('person_id',$person_id);
			$this->db->update('employees',array('accounting_id' => $accounting_id));
		}
	}
	
	function is_employee_in_quickbooks($accounting_id)
	{
		$this->db->from('employees');	
		$this->db->where('employees.accounting_id',$accounting_id);
