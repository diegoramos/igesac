...
class Item_kit extends CI_Model
{
	...
	*/
	...
	function search($search, $deleted=0,$category_id = false, $limit=20,$offset=0,$column='name',$orderby='asc', $fields = 'all')
	{
		...
		{					
			if ($this->get_custom_field($k) !== FALSE)
			{
				if ($this->get_custom_field($k,'type') != 'date')
				{
					$custom_fields[$k]=$this->db->dbprefix('item_kits').".custom_field_${k}_value LIKE '".$this->db->escape_like_str($search)."%'";
				}
				else
				{							
		...
		if ($fields == 'all')
		{
			if ($search)
			{
					...
					"%' or item_kit_number LIKE '".(!$this->config->item('speed_up_search_queries') ? '%' : '').$this->db->escape_like_str($search)."%'".
					"or product_id LIKE '".(!$this->config->item('speed_up_search_queries') ? '%' : '').$this->db->escape_like_str($search)."%' or
					".$this->db->dbprefix('tags').".name LIKE '".(!$this->config->item('speed_up_search_queries') ? '%' : '').$this->db->escape_like_str($search)."%' or
					description LIKE '".(!$this->config->item('speed_up_search_queries') ? '%' : '').$this->db->escape_like_str($search)."%' or $custom_fields) and ".$this->db->dbprefix('item_kits').".deleted=$deleted");
			}
		}
		else
		{			
	...
	function search_count_all($search, $deleted=0,$category_id = FALSE, $limit=10000)
	{
		...
		{
				$this->db->where("(".$this->db->dbprefix('item_kits').".name LIKE '".(!$this->config->item('speed_up_search_queries') ? '%' : '').$this->db->escape_like_str($search).
				"%' or item_kit_number LIKE '".(!$this->config->item('speed_up_search_queries') ? '%' : '').$this->db->escape_like_str($search)."%' or
				description LIKE '".(!$this->config->item('speed_up_search_queries') ? '%' : '').$this->db->escape_like_str($search)."%' or $custom_fields) and ".$this->db->dbprefix('item_kits').".deleted=$deleted");						
		}
		else
		{
