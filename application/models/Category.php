...
class Category extends CI_Model
{
	...
	*/
	...
	function get_category_id_from_qb_id($accounting_id)
	{
		$this->db->from('categories');
		$this->db->where('accounting_id',$accounting_id);
		$query = $this->db->get();
	...
	function get_qb_category_id($category_id)
	{
		$this->db->from('categories');
		$this->db->where('id',$category_id);
		$query = $this->db->get();
	...
	*/
    ...
    function saveUnidad($name = "", $unit = "")
    {
        if (!$this->exist('unit',array('name'=>$name)) and !$this->exist('unit',array('unit'=>$unit))) {
             if ($name!='' && $unit!='') {
                if($this->db->insert('unit',array('unit' => $unit,'name' => $name)))
                {
                    return $this->db->insert_id();
                }
            }
	...
	*/
	...
	function link_qb_category($category_id, $qb_category_id)
	{
		$this->db->where('id', $category_id);
		return $this->db->update('categories', array('accounting_id' => $qb_category_id, 'last_modified' => date('Y-m-d H:i:s')));
	}
