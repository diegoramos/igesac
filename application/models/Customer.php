...
{	
	...
	*/
	function get_all($location_id = '',$deleted = 0,$limit=10000, $offset=0,$col='last_name',$order='asc')
	{
		...
		if (!(boolean)$this->session->userdata('isadmin')) {
			$where_by_employe = 'and register_by_employee_id = '.$this->session->userdata('person_id');
		}
		...
		$data=$this->db->query("SELECT *,${people}.person_id as pid 
						...
						WHERE deleted =$deleted $location_where $where_by_employe $order_by 
						LIMIT  ".$offset.",".$limit);		
	...
	function count_all($location_id = '',$deleted = 0)
	{
		...
		$where_by_employe = '';
		if (!(boolean)$this->session->userdata('isadmin')) {
			$this->db->where('register_by_employee_id',$this->session->userdata('person_id'));
			$where_by_employe = 'and register_by_employee_id = '.$this->session->userdata('person_id');
		}		
	...
	*/
	...
	function get_customer_search_suggestions($search,$deleted = 0,$limit=25)
	{
		...
		$suggestions = array();
			...
			full_name LIKE '".$this->db->escape_like_str($search)."%') and deleted=$deleted and (location_id IS NULL or location_id = $current_location) $where_by_employe ");			
			...
			{
				$suggestions[]=array('value'=> $key, 'label' => $value['name'],'avatar'=>$value['avatar'],'subtitle'=>$value['email'],'account_number'=>$value['account_number']);		
			}
			...
			$this->db->where("(location_id IS NULL or location_id = $current_location) $where_by_employe");
			...
			for($k=1;$k<=NUMBER_OF_PEOPLE_CUSTOM_FIELDS;$k++)
			{
				if ($this->get_custom_field($k)) 
				{
					...
					{
						$this->db->select('FROM_UNIXTIME(custom_field_'.$k.'_value, "'.get_mysql_date_format().'") as custom_field, email,image_id,account_number,customers.person_id', false);
					}
					$this->db->from('customers');
					$this->db->join('people','customers.person_id=people.person_id');	
					$this->db->where('deleted',$deleted);
			...
			foreach($temp_suggestions as $key => $value)
			{
				$suggestions[]=array('value'=> $key, 'label' => $value['email'],'avatar'=>$value['avatar'],'subtitle'=>$value['email'],'account_number'=>$value['account_number']);
			}
	...
	function get_customer_buscar($search,$deleted = 0,$limit=25)
	{
		if (!trim($search))
		{
			return array();
		...
		$suggestions = array();
			...
			foreach($by_name->result() as $row)
			{
				$name_label = $row->account_number.' - '.$row->first_name.' '.$row->last_name;
				$data = array(
					'name' => $name_label,
					'email' => $row->email,
					'avatar' => $row->image_id ?  app_file_url($row->image_id) : base_url()."assets/img/user.png",
			...
			foreach($temp_suggestions as $key => $value)
			{
				$suggestions[]=array('value'=> $value['name'],'id'=>$key, 'label' => $value['name'],'avatar'=>$value['avatar'],'subtitle'=>$value['email'],'account_number'=>$value['account_number']);		
			}
	...
	*/
	  ...
	  //The queries are done as 2 unions to speed up searches to use indexes.
		...
		if ($search)
		{
				...
				else
				{
					...
					full_name LIKE '".$this->db->escape_like_str($search)."%') $where_by_employe and deleted=$deleted");		
				}				
		}
		else
		{
	...
	{
		return array('person_id','full_name','email','phone_number','balance');
	}
	...
	function get_customers_not_in_quickbooks_or_modified_since_last_sync()
	{
		...
		$data=$this->db->query("SELECT *,${people}.person_id as pid 
						...
						STRAIGHT_JOIN ".$customers." ON 										                       
						".$people.".person_id = ".$customers.".person_id
						WHERE (deleted =0 and accounting_id IS NULL) or (deleted = 0 and accounting_id IS NOT NULL and last_modified > '$last_qb_sync_date')");		
						
		return $data;
	}
	
	function is_customer_in_quickbooks($accounting_id)
	{
		$this->db->from('customers');	
		$this->db->where('customers.accounting_id',$accounting_id);
