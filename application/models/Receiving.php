...
class Receiving extends CI_Model
{
	...
	function save ($cart)
	{
		...
		$receiving_id_exist=$cart->receiving_id;
		$mode = $cart->get_mode();
		...
		//if ($this->config->item('active_facturacion')) {
			if (!$receiving_id_exist) {
				//Receipt prefix
				if ($suspended > 0) {
					$prefix=$this->Number_invoice->get_numeration_receiving('odencompra',$location_id_num);
					...
					$this->Number_invoice->save_sunat_serie_receiving($data_sunat_re);
					$this->Number_invoice->update_numeration($prefix->prefix_id,$data_numeration);
				}
				if ($suspended == 0) {
					$prefix=$this->Number_invoice->get_numeration_receiving('recepcion',$location_id_num);
					$serie=$prefix->prefijo./*$location_id_num.*/"-".$prefix->numeracion;
					$data_numeration = array(
						'numeracion' => ((int)$prefix->numeracion+1)
						 );
	...
	{		
		...
		$this->db->join('receivings_sunat', 'receivings_sunat.receiving_id = receivings.receiving_id');
	...
	function get_receivings_not_in_quickbooks_since_last_sync()
	{
		...
		$this->db->where('receivings.accounting_id', NULL);
		$this->db->where('is_po', 0);
		if(!empty($qb_setup_date)){
			$this->db->where('receivings.receiving_time > ', $qb_setup_date);
		}
		$data = $this->db->get();
		return $data;
	}
	//new added code
	function get_exchange_details($receving_id)
	{
    $this->db->from('receivings');
    $this->db->where('receiving_id',$receving_id);
    $row = $this->db->get()->row();
	...
	function link_qb_receving($receving_id, $qb_receving_id)
	{
		$this->db->where('receiving_id', $receving_id);
		return $this->db->update('receivings', array('accounting_id' => $qb_receving_id));
	}
