...
class Locations extends Secure_area implements Idata_controller
{
	...
	*/
	...
	function view($location_id=-1,$redirect=false)
	{
		...
		$result=$this->Number_invoice->get_numeration_by_location($location_id);
		$sunat_num = array();
		if ($result->num_rows()>0) {
			foreach ($result->result() as $key => $dat) {
				$sunat_num[$dat->nombre]=$dat->prefijo;
			}
		}else{
			$sunat_num = array(
				'factura' => "F00",
			    ...
			    'odencompra' => "O00",
			    'recepcion' => "R00",
			    'notacredito_factura' => "F00",
			    'notadebito_boleta' => "B00",
			    'notadebito_factura' => "F00",
			    'notacredito_boleta' => "B00",
			    'guia_remision' => "T00",
			    );
		}
		$data['sunat_num'] = $sunat_num;
...
/*	function does_have_valid_number_of_locations_for_an_additional_location($email)
	...
	function save($location_id=-1)
	{
		...
		$sunat_num = array(
			    ...
			    'odencompra' => $this->input->post('odencompra'),
			    'recepcion' => $this->input->post('recepcion'),
			    'notacredito_factura' => $this->input->post('notacredito_factura'),
			    'notadebito_boleta' => $this->input->post('notadebito_boleta'),
			    'notadebito_factura' => $this->input->post('notadebito_factura'),
			    'notacredito_boleta' => $this->input->post('notacredito_boleta'),
...
/*		if ($location_id == -1)
		...
		if($this->Location->save($location_data,$location_id) && $this->Location->assign_employees_to_location($location_id != -1 ? $location_id : $location_data['location_id'],$employees))
		{
			...
			if($location_id==-1)
			{
				foreach ($sunat_num as $keys => $val) {
					$prefix_data = array('nombre' => $keys,'location_id'=>$location_data['location_id'],'numeracion'=>1);
					foreach ($val as $key => $value) {
						$prefix_data['name_sales']=$key;
						$prefix_data['prefijo']=$value;
						$this->Number_invoice->insert_prefix($prefix_data);
					}
				}
			...
			else //previous item
			{
				...
				if ($result->num_rows()>0) {
					foreach ($sunat_num as $keys => $val) {
						$prefix_where = array('nombre' => $keys,'location_id'=>$location_id);
						foreach ($val as $key => $value) {
							$this->Number_invoice->update_prefix(array('prefijo' => $value),$prefix_where);
						}
					}
				}else{
					foreach ($sunat_num as $keys => $val) {
						$prefix_data = array('nombre' => $keys,'location_id'=>$location_id,'numeracion'=>1);
						foreach ($val as $key => $value) {
							$prefix_data['name_sales']=$key;
							$prefix_data['prefijo']=$value;
							$this->Number_invoice->insert_prefix($prefix_data);
