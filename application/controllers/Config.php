...
class Config extends Secure_area 
{
	...
	function index()
	{
		...
		$data['tipo_cambio'] = $this->Appconfig->get_tipo_cambio();
		if ($data['tipo_cambio']!=null) {
			$data['one_dolar']	=	round(1/$data['tipo_cambio']->venta,3);
		} else {
			$data['one_dolar']	=	null;
		}
		...
		// Code to get all the Customers start
		$all_customer_data = array();
		$customers = $this->Customer->get_all()->result();
		if (!empty($customers)) {
			foreach ($customers as $customer) {
				$all_customer_data[$customer->person_id] = $customer->full_name;
			}
		}
		$data['all_customer_data']=$all_customer_data;
		// Code to get all the Customers ends

		// Code to get all the items start
		$all_item_data = array();
		$items = $this->Item->get_all()->result();
		if (!empty($items)) {
			foreach ($items as $item) {
				$all_item_data[$item->accounting_id] = $item->name;
			}
		}
		$data['all_item_data']=$all_item_data;
		// Code to get all the items ends

		// Code to get all the categories start
		$all_category_data = array();
		$categories = $this->Category->get_all_categories_and_sub_categories_as_tree();
		if (!empty($categories)) {
			foreach ($categories as $category) {
				$all_category_data[$category->name] = $category->name;
			}
		}
		$data['all_category_data']=$all_category_data;
		// Code to get all the categories ends

		// Code to get chart of accounts from Qb Start
		$arrayAccountsQb = array();

		// Variable to store the tax name and id to set the default tax in chart of account
		$taxArray = array();
		try {
			if ($this->config->item('quickbooks_access_token')) {
				...
				$offset = 0;
				$per_page = 100;
				$arrayAccountTypes = array('all', 'Bank', 'Credit Card', 'Other Current Liability', 'Other Current Asset', 'Income', 'Other Current Asset', 'Expense', 'Cost of Goods Sold', 'bank_and_credit_card');
				// initialize the array with required account types
				foreach($arrayAccountTypes as $type) {
					$arrayAccountsQb[$type] = array();
				}
				// Check if the QB service is enabled or not.
				if ($dataService) {
					while (1) {
						// Refresh Tokens if it is near to expire (i.e before 5 minutes)
						refresh_tokens(0);
						$allAccounts = $dataService->FindAll('Account', $offset, $per_page);
						$offset += $per_page;
						if (!empty($allAccounts) and count($allAccounts) > 0) {
							foreach ($allAccounts as $account) {
								...
								//flag to check if the account is valid to be shown in the respective dropdown options, if it is not we will set it to false and the account won’t be added in the array
								$flagValidAccount = true;
								// Code to get the Accounts by Account Type for Chart of account
								if (!isset($arrayAccountsQb[$accountType])) {
									$arrayAccountsQb[$accountType] = array();
								}
								// build array of account based on their types, so that we can set those as dropdown options
								$arrayAccountsQb['all'][$accountId] = $accountName;
								
								/** 
								* The income account we need for creating inventory items, should have detail type "SalesOfProductIncome". 
								* The Other Current Asset we need for creating inventory items, should have detail type "Inventory". 
								* bank_and_credit_card will contains the list of account which is having bank and credit cards accounts 
								* In quickbooks, it do not show rest of the accounts which have different account type or detail type, 
								* So to prevent any error while exporting inventory items, we are going to show only valid accounts for user to select
								*/
								if (($accountType == 'Income') && ($accountSubType !== 'SalesOfProductIncome')) {
									$flagValidAccount = false;
								}
								if (($accountType == 'Other Current Asset') && ($accountSubType !== 'Inventory')) {
									$flagValidAccount = false;
								}
								if (($accountType == 'Bank') || ($accountType == 'Credit Card')) {
									$arrayAccountsQb['bank_and_credit_card'][$accountId] = $accountName;
								}
								if ($flagValidAccount == true) {
									$arrayAccountsQb[$accountType][$accountId] = $accountName;
								}
							}
						} else {
							break;
					...
					if ($this->config->item('default_country_id') != US_CODE) {
						// Get all the tax rates from the QBO
						$tax_offset = 0;
						$tax_per_page = 100;
						while (1) {
							// Refresh Tokens if it is near to expire (i.e before 5 minutes)
			...
			$this->_log("Error in fetching Qb accounts: " . $e->getMessage() . "");
			$this->_save_log();
		}

		$data['arrayAccountsQb']=$arrayAccountsQb;
		// Code to get chart of accounts ends
		
		$data['taxArray']=$taxArray;
	...
	function save()
	{
		...
		$errorMessage = "";
        if ($this->config->item('quickbooks_access_token')) {
			// if quickbooks has been enabled then below fields are required ones
            // we need these for creating items on fly
            ...
            if (empty($revenue_account_id) || !isset($revenue_account_id)) {
                $errorMessage = lang('config_revenue_id');
            } else if (empty($asset_account_id) || !isset($asset_account_id)) {
                $errorMessage = lang('config_asset_id');
            } else if (empty($expense_account_id) || !isset($expense_account_id)) {
                $errorMessage = lang('config_expense_id');
            }
        }
      ...
      curl_close($ch);		
		...
		// Check if qb_setup_date contain any value if not then it will update it with current date and time
		$qb_setup_date = $this->Appconfig->get('qb_setup_date');
		if(empty($qb_setup_date)) {
			$qb_setup_date = date('Y-m-d H:i:s');
		}
		...
		'ruc_company'=>$this->input->post('ruc_company'),
		'active_facturacion' => $this->input->post('active_facturacion') ? 1 : 0,
		//'usuariosol' => $this->input->post('usuariosol'),
		//'clavesol' => $this->input->post('clavesol'),
		'ruta_sunat' => $this->input->post('ruta_sunat'),
		...
		'announcement_oc'=>$this->input->post('announcement_oc'),		
		...
		'revenue_account_id' => $this->input->post('revenue_account_id') ? $this->input->post('revenue_account_id') : '',
		'asset_account_id' => $this->input->post('asset_account_id') ? $this->input->post('asset_account_id') : '',
		'expense_account_id' => $this->input->post('expense_account_id') ? $this->input->post('expense_account_id') : '',
		'refund_credit_account' => $this->input->post('refund_credit_account') ? $this->input->post('refund_credit_account') : '',
		'refund_cash_account' => $this->input->post('refund_cash_account') ? $this->input->post('refund_cash_account') : '',
		'refund_debit_card_account' => $this->input->post('refund_debit_card_account') ? $this->input->post('refund_debit_card_account') : '',
		'refund_credit_card_account' => $this->input->post('refund_credit_card_account') ? $this->input->post('refund_credit_card_account') : '',
		'refund_check_account' => $this->input->post('refund_check_account') ? $this->input->post('refund_check_account') : '',
		'refund_deposit_account' => $this->input->post('refund_deposit_account') ? $this->input->post('refund_deposit_account') : '',
		'expense_account' => $this->input->post('expense_account') ? $this->input->post('expense_account') : '',
		'commission_credit_account' => $this->input->post('commission_credit_account') ? $this->input->post('commission_credit_account') : '',
		'commission_debit_account' => $this->input->post('commission_debit_account') ? $this->input->post('commission_debit_account') : '',
		'store_account' => $this->input->post('store_account') ? $this->input->post('store_account') : '',
		'default_customer_id' => $this->input->post('default_customer_id') ? $this->input->post('default_customer_id') : '',
		'expense_bank_credit_account' => $this->input->post('expense_bank_credit_account') ? $this->input->post('expense_bank_credit_account') : '',
		// Store the default tax id for non usa countries
		'default_tax_id' => $this->input->post('default_tax_id') ? $this->input->post('default_tax_id') : '',
		// Store the default store account tax id for non usa countries
		'default_store_account_tax_id' => $this->input->post('default_store_account_tax_id') ? $this->input->post('default_store_account_tax_id') : '',
		// Store the country code to check usa or non usa
		'default_country_id' => $countryID,
		'qb_setup_date' => $qb_setup_date,
  ...
  /**
   * funcion para verificar valides de la ruta del webservice
   * probarSunat
   */
  public function probarSunat(){
	if ($this->config->item('active_facturacion')) {
		$ruta =  $this->config->item('ruta_sunat');
		if($ruta!=''){
			$token = $this->config->item('token');
			if($token!=''){
				//Aqui va toda la logia del envio para la prueba
				$sales_data_sunat = array(
					'tipo_de_comprobante' 			=> 'prueba'
				);
				...
				echo json_encode(array('status'=>FALSE,'message'=>"Por favor ingresar el token de seguridad."));
			}
		}else{
			echo json_encode(array('status'=>FALSE,'message'=>"Por favor ingresar la ruta del webservice."));
		}
	}else{
		echo json_encode(array('status'=>FALSE,'message'=>"Por favor activar la opcion facturación electrónica."));
	}
  }
}
?>
