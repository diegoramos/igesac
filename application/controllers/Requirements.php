...
class Requirements extends Secure_area implements Idata_controller
{
    ...
    function sorting() {
        ...
        $this->load->library('pagination');$this->pagination->initialize($config);
        $data['pagination'] = $this->pagination->create_links();
        $data['manage_table'] = get_requirements_manage_table_data_rows($table_data, $this);
    ...
    function search() {
        ...
        $config['base_url'] = site_url('requirements/search');
        $config['total_rows'] = $this->Requirement->search_count_all($search,$deleted);
        $config['per_page'] = $per_page;
        $this->load->library('pagination');$this->pagination->initialize($config);
        $data['pagination'] = $this->pagination->create_links();
    ...
    function view($requirement_id = -1, $redirect_code = 0) {
        $this->check_action_permission('add_update');
        $logged_employee_id = $this->Employee->get_logged_in_employee_info()->person_id;
        $data['requirement_info'] = $this->Requirement->get_info($expense_id);
    ...
    function delete() {
        ...
        $requirement_to_delete = $this->input->post('ids');
        if ($this->Requirement->delete_list($requirement_to_delete)) {
    ...
    function undelete() {
        $this->check_action_permission('delete');
        $requirement_to_delete = $this->input->post('ids');
        if ($this->Requirement->undelete_list($requirement_to_delete)) {
