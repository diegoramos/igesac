...
class Items extends Secure_area implements Idata_controller
{
	...
	function save_unit($category_id = FALSE)
	{	
		$this->check_action_permission('manage_categories');
		//$update = $category_id ? true : false;
		...
		//$this->load->model('Unit_model');
		if ($unit_id = $this->Unit_model->saveUnidad($name, $abbreviation))
		{
			$unit_data=$this->Unit_model->getDataAllUnidades();
			$units = array();
			foreach($unit_data as $key=>$value)
			{
				$units[] = array('value'=> H($key),'text'=> H($value));
			}
			echo json_encode(array('success'=>true,'message'=>lang('items_unit_successful_adding').' '.H($name), 'units' => $units, 'selected' => $unit_id));
		}
		else
		{
			echo json_encode(array('success'=>false,'message'=>lang('items_unit_successful_error')));
		}
	}
	...
	*/
	...
	{	
		...
		//$this->load->model('Unit_model');
		/*Esto es otro ingresado por yony*/
		if ($this->Unit_model->getDataUnidadById($data['item_info']->unit_id)!='') {
			$data['units'] = $this->Unit_model->getDataUnidadById($data['item_info']->unit_id);
		}else{
			$data['unit'] = 'Ninguno';
		}
	...
	function bulk_edit()
	{
		...
		$data['suppliers'] = $suppliers;
		$data['unit'][''] = lang('common_do_nothing');
		$data['categories'][''] = lang('common_do_nothing');
		$data['manufacturers'][''] = lang('common_do_nothing');

		$units = array('' => lang('common_do_nothing'), '-1' => lang('common_none'));
		foreach ($this->Unit_model->get_all() as $id => $row)
		{
			$units[$id] = $row['abbreviation'];
		}
		$data['unit'] = $units;
			...
			foreach($item_variations as $item_variation_id => $item_variation)
			{
				...
				{			
					...
					(
						...
						'location_id'=> $location_id,
						'inventory_sync_needed'=> INVENTORY_SYNC_NEEDED, // inventory_sync_needed is added to check the current status of quantity of an item in inventory table
	...
	function bulk_update()
	{
		...
		foreach($_POST as $key=>$value)
		{
			...
			if ($key == 'supplier_id' || $key =='manufacturer_id' || $key == 'unit_id')
			{
				if ($value!='')
				{
	...
	{			
		...
		foreach ($data as $r) 
		{
			...
			$to_export[lang('common_unit')][] = isset($units[$r->unit_id]) ? $units[$r->unit_id] : '';
	...
	{		
		...
		$fields[] = array('Name' => lang('common_unit'), 'key' => 'unit_id');
	...
	function complete_excel_import()
	{
		...
		$this->units_map = array();
		
		foreach($this->Unit_model->get_all() as $id => $row)
		{
		 	$this->units_map[strtoupper($row['abbreviation'])] = $id;
		}
	...
	{	//$row added for logging warnings if we decide to
		...
		if ($key == 'unit_id')
		{
			if ($value)
			{
				...
				if (isset($this->units_map[strtoupper($value)]))
				{
					$unit_id = $this->units_map[strtoupper($value)];
				}	
				else
				{
					$unit_id = $this->Unit_model->save($value);
					$this->units_map[strtoupper($value)] = $unit_id;
				}
				return $unit_id;
			}	
		}
