    ...
    public function index_get($manufacturers_id = NULL)
    {
			...
			if ($manufacturers_id === NULL)
      {
      	$search = $this->input->get('search');
				...
				if ($search)
				{
					$manufacturers = $this->Manufacturer->search($search, 0, $limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'name','asc');
					$total_records = $this->Manufacturer->search_count_all($search, 0,10000);
				}
				else
				{
					$manufacturers = $this->Manufacturer->get_all($limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'id','desc');
