    ...
    public function index_get($person_id = NULL)
    {
			...
			if ($person_id === NULL)
      {
      	...
      	$search_field = $this->input->get('search_field');
				...
				{
					$customers = $this->Customer->get_all($location_id,0,$limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0)->result();
    ...
    private function _create_customer($customer_request)
    {
    	 $this->load->model('Customer');
			...
			$customer_data=array(
				...
				'taxable'=>isset($customer_request['taxable']) && $customer_request['taxable'] ? 1 : 0,
