    ...
    function __construct()
    {
		...
		function index_get($delivery_id = NULL)
		{
      	...
      	$search = $this->input->get('search');
				...
				if ($search)
				{
					$deliveries = $this->Delivery->search($search, 0, array(),$limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'estimated_shipping_date','asc',$location_id)->result();
					$total_records = $this->Delivery->search_count_all($search, 0,array(),10000,$location_id);
				}
				else
				{
					
					$deliveries = $this->Delivery->get_all(0,$limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'estimated_shipping_date','asc', array(),$location_id)->result();
