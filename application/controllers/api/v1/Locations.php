    ...
    public function index_get($location_id = NULL)
    {
			...
			if ($location_id === NULL)
      {
      	...
      	$search_field = $this->input->get('search_field');
				...
				if ($search)
				{
					$locations = $this->Location->search($search, 0, $limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'name','asc',$search_field)->result();
					$total_records = $this->Location->search_count_all($search, 0,10000,$search_field);
				}
				else
				{
					$locations = $this->Location->get_all(0,$limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0)->result();
