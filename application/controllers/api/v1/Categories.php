    ...
    public function index_get($categories_id = NULL)
    {
			...
			if ($categories_id === NULL)
      {
      	$search = $this->input->get('search');
				...
				if ($search)
				{
					$categories = $this->Category->search($search, 0, $limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'name','asc');
					$total_records = $this->Category->search_count_all($search, 0,10000);
				}
				else
				{
					$categories = $this->Category->get_all(NULL,FALSE,$limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'id','desc');
				...
				{
						$categories_return[] = $this->_categories_result_to_array(array('id' => $id,'name' => $categories['name']));
				}
