    ...
    public function index_get($giftcard_id = NULL)
    {
			...
			if ($giftcard_id === NULL)
      {
      	$search = $this->input->get('search');
				...
				if ($search)
				{
					$giftcards = $this->Giftcard->search($search, 0, $limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'giftcard_number','asc')->result();
					$total_records = $this->Giftcard->search_count_all($search, 0,10000);
				}
				else
				{
					$giftcards = $this->Giftcard->get_all(0,$limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0)->result();
