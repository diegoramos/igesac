    ...
    public function index_get($attributes_id = NULL)
    {
			...
			if ($attributes_id === NULL)
      {
      	$search = $this->input->get('search');
				...
				if ($search)
				{
					$attributes = $this->Item_attribute->search($search, 0, $limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'name','asc')->result_array();
					$total_records = $this->Item_attribute->search_count_all($search, 0,10000);
				}
				else
				{
					$attributes = $this->Item_attribute->get_all($limit!==NULL ? $limit : 20, $offset!==NULL ? $offset : 0,'id','desc')->result_array();
