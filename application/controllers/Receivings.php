...
{	
			...
			if($this->input->post("name"))
			{
				...
				if ($item = $this->cart->get_item($line))
				{
					try
					{
						...
						{
							if ($variable == 'quantity' && $quantity != (int)$quantity)
							{
	...
	function complete()
	{
		...
		if ($this->cart->suspended==0) {
			$retur_num=$this->Number_invoice->save_sunat_serie_by_id_receiving($receiving_id_raw,0);
			if ($retur_num->serie!='') {
				$data['receiving_id']=$retur_num->serie;
				$data['receipt_title']=($retur_num->name_sales!='')? lang($retur_num->name_sales):lang('receivings_receipt');
			}else{
				$retur_num=$this->Number_invoice->save_sunat_serie_by_id_receiving($receiving_id_raw,1);
				if ($retur_num->serie!='') {
					$data['receiving_id']=$retur_num->serie;
					$data['receipt_title']=($retur_num->name_sales!='')? lang($retur_num->name_sales):lang('receivings_receipt');
				}else{
	...
	function email_receipt($receiving_id)
	{
		...
		if ($data['suspended'] > 0)
		{
			$retur_num=$this->Number_invoice->save_sunat_serie_by_id_receiving($receiving_id,1);
			if ($retur_num->serie!='') {
				$data['receiving_id']=$retur_num->serie;
				$data['receipt_title']=($retur_num->name_sales!='')? lang($retur_num->name_sales):lang('receivings_receipt');
			}else{
			...
			$retur_num=$this->Number_invoice->save_sunat_serie_by_id_receiving($receiving_id,0);
			if ($retur_num->serie!='') {
				$data['receiving_id']=$retur_num->serie;
				$data['receipt_title']=($retur_num->name_sales!='')? lang($retur_num->name_sales):lang('receivings_receipt');
			}else{
	...
	function suspend()
	{
		...
		if ($this->config->item('show_receipt_after_suspending_sale') || $is_po)
		{
			...
			$this->cart->destroy();
			$this->cart->save();
			redirect('receivings/receipt_op/'.$receiving_id_raw);
		}
		else
		{
			$this->cart->destroy();
	...
	function receipt_pdf($receiving_id)
	{
		$receipt_cart = PHPPOSCartRecv::get_instance_from_recv_id($receiving_id);
		if ($this->config->item('sort_receipt_column'))
		{
		...
		//$this->load->view("receivings/receipt_op",$data);
		//$receipt_cart->destroy();
	...
	function receipt_op($receiving_id)
	{
		$receipt_cart = PHPPOSCartRecv::get_instance_from_recv_id($receiving_id);
		if ($this->config->item('sort_receipt_column'))
		{
	...
	function receipt_op_pdf($receiving_id)
	{
		$receipt_cart = PHPPOSCartRecv::get_instance_from_recv_id($receiving_id);
		if ($this->config->item('sort_receipt_column'))
		{
		...
		//$this->load->view("receivings/receipt_op",$data);
		//$receipt_cart->destroy();
		$html = $this->load->view("receivings/receipt_op_pdf",$data,TRUE);
		$filename=date('Ymdhis');
		$mpdf = new \Mpdf\Mpdf(['margin_left' => 8,
								'margin_right' => 8,
								'margin_top' => 8,
	...
	function edit($receiving_id)
	{
		...
		//nuevo
		if ($data['receiving_info']['suspended'] > 0)
		{
			$retur_num=$this->Number_invoice->save_sunat_serie_by_id_receiving($receiving_id,1);
			if ($retur_num->serie!='') {
				$data['serie']=$retur_num->serie;
			}else{
				$data['serie']='RECV '.$receiving_id;
			}
		}else{
			$retur_num=$this->Number_invoice->save_sunat_serie_by_id_receiving($receiving_id,0);
			if ($retur_num->serie!='') {
	...
	{		
		...
		//nuevo
		if ($this->cart->receiving_id) {
			if ($this->cart->suspended > 0)
			{
				$retur_num=$this->Number_invoice->save_sunat_serie_by_id_receiving($this->cart->receiving_id,1);
				if ($retur_num->serie!='') {
					$data['serie']=$retur_num->serie;
				}else{
					$data['serie']='RECV '.$this->cart->receiving_id;
				}
			}else{
				$retur_num=$this->Number_invoice->save_sunat_serie_by_id_receiving($this->cart->receiving_id,0);
				if ($retur_num->serie!='') {
					$data['serie']=$retur_num->serie;
				}else{
					$data['serie']='RECV '.$this->cart->receiving_id;
