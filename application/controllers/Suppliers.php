...
{	
	...
	*/
	...
	//Buscar ruc de la sunat
	public function buscarRuc(){
		//require (APPPATH."libraries/sunat/autoload.php");
		error_reporting(E_ALL);
		ini_set('display_errors', TRUE);
		ini_set('display_startup_errors', TRUE);
		header('Content-Type: text/plain');
		$dni=$this->input->post("nruc");
		if((strlen( $dni )==11 || strlen( $dni )==8) && $dni!="" ){
			require (APPPATH."libraries/SunatPHP/src/autoload.php");
			$company = new \Sunat\Sunat( true, true );
			$search1 = $company->search( $dni );
			if( $search1->success == true )
			{
				$search1->type="sunat";
				if (isset($search1->result->cantidad_trabajadores)) {
					unset($search1->result->cantidad_trabajadores);
				}
				if (isset($search1->result->representantes_legales)) {
					unset($search1->result->representantes_legales);
				}
		...
		if( strlen( $dni )==8 && $dni!="" )
		{
			require (APPPATH."libraries/datos-peru/src/autoload.php");
			$essalud = new \EsSalud\EsSalud();
			$search1 = $essalud->search( $dni );
			if( $search1->success == true )
			{	$search1->type="nosunat";
				echo json_encode($search1);
				exit();
			}
