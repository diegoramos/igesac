...
class Home extends Secure_area 
{
	...
	{		
		...
		///New cambio
		$da['tipo_cambio'] = $this->Appconfig->get_tipo_cambio();
		if ($da['tipo_cambio']!=null) {
			$da['one_dolar']	=	round(1/$da['tipo_cambio']->venta,4);
		} else {
			$da['one_dolar']	= null;
		}
		$data['tipo_cambio'] = $da['tipo_cambio'];
		$datas = array();
		foreach ($this->Appconfig->get_exchange_rates()->result() as $key => $dat) {
			if($da['one_dolar']!=null){
				if ($dat->currency_code_to=='USD') {
					$datas['id']=$dat->id;
					$datas['exchange_rate']=$da['one_dolar'];
				}
			}
		}
		$this->Appconfig->update_exchange_rates($datas);
	...
	{	
		...
		$progress = $this->Quickbooks->get_sync_progress();
