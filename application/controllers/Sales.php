...
class Sales extends Secure_area
{
	...
	function nuevo($sale_id){//yony
		$receipt_cart = PHPPOSCartSale::get_instance_from_sale_id($sale_id);
		$customer_id = $receipt_cart->customer_id;
		$this->cart->select_customer($customer_id);
		$this->cart->type_comprobante = isset($receipt_cart->type_comprobante)?$receipt_cart->type_comprobante:3;
		foreach ($receipt_cart->get_items() as $key => $item) {
			$barcode_scan_data = $item->item_id."|FORCE_ITEM_ID|";
	...
	function set_type_comprobante()
    {
        $this->cart->type_comprobante = $this->input->post('tipo_comprobante');
        $this->cart->save();
    }
	...
	function declined()
	{
		...
		$data['receipt_title']= $this->config->item('override_receipt_title') ? $this->config->item('override_receipt_title') : lang('sales_note');
		...
		if($customer_id)
		{
			...
			$data['customer_account']= ($cust_info->account_number==''  ? '': $cust_info->account_number);
	...
	{	
		...
		{
			if ($this->cart->type_comprobante!=4) {
				$cliente=$this->Customer->get_info($this->cart->customer_id);
				$customer_account_number=strlen($cliente->account_number);
				if ($customer_account_number==8 && $this->cart->type_comprobante!=3) {
					$this->_reload(array('error' =>'No es posible realizar una factura con Nro. DNI.'), false);
					return;
				}
				if (($cliente->account_number=='00000000' || $cliente->account_number==null)  && ($this->cart->type_comprobante==1)) {
					$this->_reload(array('error' =>'Para realizar una factura ingrese Nro. RUC del cliente'), false);
					return;
				}

				if ($this->cart->type_comprobante==3) {
					if ($cliente->account_number=='00000000' || $cliente->account_number==null) {
						if ($this->cart->get_total()>700) {
							$this->_reload(array('error' =>'Para realizar venta de este monto se requiere Nro. DNI'), false);
							return;
						}
					}
				}
			...
			if ($this->cart->type_comprobante==1) {
				$this->_reload(array('error' =>'Por favor seleccionar un cliente para realizar una factura.'), false);
				return;
			}
		}
		...
		if ($this->cart->suspended==0) {
			$retur_num=$this->Number_invoice->save_sunat_serie_by_id_sale($sale_id_raw,0);
			if ($retur_num->serie!='') {
				$data['sale_id']=$retur_num->serie;
				$data['receipt_title']=($retur_num->name_sales!='')? lang($retur_num->name_sales):lang('sales_note');
			}else{
				/*$retur_num=$this->Number_invoice->save_sunat_serie_by_id_sale($sale_id_raw,1);
				if ($retur_num->serie!='') {
					$data['sale_id']=$retur_num->serie;
					$data['receipt_title']=($retur_num->name_sales!='')? lang($retur_num->name_sales):lang('sales_note');
				}else{*/
		...
		{			
			if ($this->cart->email_receipt && !empty($cust_info->email))
			{
				...
				$this->email->subject($this->config->item('emailed_receipt_subject') ? $this->config->item('emailed_receipt_subject') : lang('sales_note'));
		...
		$params['data'] = "ruc|nro|igv|fecha";
		$params['level'] = 'H';
		$params['size'] = 3;
		$params['savename'] = FCPATH.'uploads/qr.png';
		$data['qr'] = $this->ciqrcode->generate($params);
		
		$this->load->view("sales/receipt",$data,FALSE);
	...
	function email_receipt($sale_id)
	{
		...
		{
			$retur_num=$this->Number_invoice->save_sunat_serie_by_id_sale($sale_id,1);
			if ($retur_num->serie!='') {
					$data['sale_id']=$retur_num->serie;
					$data['receipt_title']=($retur_num->name_sales!='')? lang($retur_num->name_sales):lang('sales_note');
			}else{
		...
		if (!empty($data['customer_email']))
		{
			...
			$this->email->subject($sale_info['suspended'] == 2 ? lang('common_estimate') : ($this->config->item('emailed_receipt_subject') ? $this->config->item('emailed_receipt_subject') : lang('sales_note')));
			$this->email->message($this->load->view("sales/receipt_email",$data, true));	
			$this->email->send();
		}
	}
	...
	function quotation($sale_id)
	{
		...
		$data['amount_change']=$receipt_cart->get_amount_due() * -1;
		$data['employee']=$emp_info->first_name.' '.$emp_info->last_name.($sold_by_employee_id && $sold_by_employee_id != $sale_info['employee_id'] ? '/'. $sale_emp_info->first_name.' '.$sale_emp_info->last_name: '');
		$data['phone_number_employee'] = $emp_info->phone_number;
		$data['email_employee'] = $emp_info->email;
		$data['ref_no'] = $sale_info['cc_ref_no'];
		$data['auth_code'] = $sale_info['auth_code'];
		$data['discount_exists'] = $this->_does_discount_exists($data['cart_items']);
		$data['disable_loyalty'] = 0;
		...
		$this->load->view("sales/receipt_quotation",$data);
	}

	function quotation_pdf($sale_id)
	{
		$receipt_cart = PHPPOSCartSale::get_instance_from_sale_id($sale_id);
		if ($this->config->item('sort_receipt_column'))
		{
		...
		//$this->load->view("sales/receipt_quotation_pdf",$data);
		$html = $this->load->view("sales/receipt_quotation_pdf",$data,TRUE);
		$filename=date('Ymdhis');
		$mpdf = new \Mpdf\Mpdf(['margin_left' => 8,
								'margin_right' => 8,
								'margin_top' => 8,
								'margin_bottom' => 8,
								'format' => 'Letter'
	...
	function edit($sale_id)
	{
		...
		//nuevo
		$anular=FALSE;
		if ($data['sale_info']['suspended'] > 0)
		{
			$retur_num=$this->Number_invoice->save_sunat_serie_by_id_sale($sale_id,1);
			if ($retur_num->serie!='') {
				$data['serie']=$retur_num->serie;
			}else{
				$data['serie']=$this->config->item('sale_prefix').' '.$sale_id;
			}
		}else{
			$retur_num=$this->Number_invoice->save_sunat_serie_by_id_sale($sale_id,0);
			if ($retur_num->serie!='') {
		...
		$datetime1 = date_create($data['sale_info']['sale_time']);
		$datetime2 = date_create(date('Y-m-d H:i:s'));
		$interval = date_diff($datetime1, $datetime2);
		$data['anular_sunat']=FALSE;
		if ($anular) {
			if ($interval->d>7) {
				$data['anular_sunat']=TRUE;
			}
		}
	...
	function delete($sale_id)
	{
		...
		$can_delete = TRUE;
		$mensaje_motivo_anulacion=$this->input->post('motivo_anulacion');
		...
		{
			if ($this->Sale->delete($sale_id,false,$mensaje_motivo_anulacion))
			{			
				$data['success'] = true;
				if ($this->input->post('clear_sale'))
				{
	...
	{	
		...
		//nuevo
		if ($this->cart->get_previous_receipt_id()) {
			if ($this->cart->suspended > 0)
			{
				$retur_num=$this->Number_invoice->save_sunat_serie_by_id_sale($this->cart->get_previous_receipt_id(),1);
				if ($retur_num->serie!='') {
					$data['serie']=$retur_num->serie;
				}else{
					$data['serie']=$this->config->item('sale_prefix').' '.$this->cart->get_previous_receipt_id();
				}
			}else{
				$retur_num=$this->Number_invoice->save_sunat_serie_by_id_sale($this->cart->get_previous_receipt_id(),0);
				if ($retur_num->serie!='') {
					$data['serie']=$retur_num->serie;
				}else{
					$data['serie']=$this->config->item('sale_prefix').' '.$this->cart->get_previous_receipt_id();
  ...
  function cancel_sale()
  {
	...
	function suspend($suspend_type = 1)
	{
		...
		{
			redirect('sales/quotation/'.$sale_id);
			//redirect('sales/receipt/'.$sale_id); original
		}
		else
		{
