...
function customer_name_data_function($person)
{
	...
	$data['link']= site_url('reports/generate/specific_customer?report_type=complex&report_date_range_simple=CUSTOM&start_date='.$six_months_ago.'&start_date_formatted='.date(get_date_format().' '.get_time_format(), strtotime($six_months_ago)).'&end_date='.$today.'&end_date_formatted='.date(get_date_format().' '.get_time_format(), strtotime(date('Y-m-d'))).'&customer_id='.$person->person_id.'&sale_type=all&export_excel=0');
...
function supplier_name_data_function($person)
{
	...
	$six_months_ago = date('Y-m-d', strtotime('-6 months'));
	$today = date('Y-m-d').'%2023:59:59';
	$data['link']= site_url('reports/generate/specific_supplier?report_type=complex&report_date_range_simple=CUSTOM&start_date='.$six_months_ago.'&start_date_formatted='.date(get_date_format().' '.get_time_format(), strtotime($six_months_ago)).'&end_date='.$today.'&end_date_formatted='.date(get_date_format().' '.get_time_format(), strtotime(date('Y-m-d'))).'&supplier_id='.$person->person_id.'&receiving_type=all&export_excel=0');
