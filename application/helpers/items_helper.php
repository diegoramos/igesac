...
function get_item_variations_barcode_data($item_variation_ids)
{
	...
	foreach ($item_variation_ids as $item_variation_id)
	{
		...
		$barcode_name = $CI->config->item('hide_name_on_barcodes') ? '' : $item_variation_info->name.' '.$item_info->name.($item_info->size ? ' ('.$item_info->size.')' : '');
		...
		if($CI->config->item('barcode_price_include_tax'))
		{
			...
			{
				$result[] = array('name' => !$hide_prices ? (($is_item_location_promo || $is_item_promo ? '<span style="text-decoration: line-through;font-weight:bold;">'.to_currency($regular_item_price).'</span> ' : ' ').'<span style="font-weight:bold;">'.to_currency($item_price).'</span> '.$barcode_name) : $barcode_name , 'id'=> $barcode_number);
			}
			else
			{				
				$result[] = array('name' => !$hide_prices ? (($is_item_location_promo || $is_item_promo ? '<span style="text-decoration: line-through;font-weight:bold;">'.to_currency(get_price_for_item_including_taxes($item_id,$regular_item_price)).'</span> ' : ' ').'<span style="font-weight:bold;">'.to_currency(get_price_for_item_including_taxes($item_id,$item_price)).'</span> '.$barcode_name) : $barcode_name, 'id'=> $barcode_number);
	  	}
	  }
	  else
	  {
		if ($item_info->tax_included)
		{
		    $result[] = array('name' => !$hide_prices ? (($is_item_location_promo || $is_item_promo ? '<span style="text-decoration: line-through;font-weight:bold;">'.to_currency(get_price_for_item_excluding_taxes($item_id, $regular_item_price)).'</span> ' : ' ').'<span style="font-weight:bold;">'.to_currency(get_price_for_item_excluding_taxes($item_id, $item_price)).'</span> '.$barcode_name) : $barcode_name, 'id'=> $barcode_number);
		}
		else
		{
			$result[] = array('name' => !$hide_prices ? (($is_item_location_promo || $is_item_promo ? '<span style="text-decoration: line-through;font-weight:bold;">'.to_currency($regular_item_price).'</span> ' : ' ').'<span style="font-weight:bold;">'.to_currency($item_price).'</span> '.$barcode_name) : $barcode_name, 'id'=> $barcode_number);
