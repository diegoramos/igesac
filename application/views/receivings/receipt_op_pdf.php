...
<body>
	<center class="wrapper" style="width:100%;table-layout:fixed;-webkit-text-size-adjust:100%;-ms-text-size-adjust:100%;" >
		<div class="webkit" style="max-width:100%;background-color:#FFFFFF;margin-top:30px;border-radius:6px;border-width:1px;border-style:solid;border-color:#DCE0E6;" >
			<!--[if (gte mso 9)|(IE)]>
			...
			<table class="outer" align="center" style="border-spacing:0;font-family:'Helvetica Neue', Helvetica, Arial, sans-serif;color:#555555;font-size:13px;Margin:0 auto;width:100%;max-width:100%;" >
									...
									<table width="100%" style="border-spacing:0;font-family:'Helvetica Neue', Helvetica, Arial, sans-serif;color:#555555;font-size:13px;" >
										<tr>
											<td class="inner contents receipt-header" align="center" style="padding-top:10px;padding-bottom:10px;padding-right:10px;padding-left:10px;width:100%;height:48px;background-color:#2196F3;color:#FFFFFF;border-top-left-radius:6px;border-top-right-radius:6px;text-align:center !important;" >
												...
												<?php echo $is_po ? lang('receivings_purchase_order') : H($receipt_title); ?> #<?php echo $is_po ? (($receiving_id!='')? $receiving_id : H($receiving_id_raw)) : H($receiving_id); ?>
														<?php } else { 
															?>
											...
											<tr>
												<td class="inner" style="padding-top:10px;padding-bottom:10px;padding-right:10px;padding-left:10px;" >
													<table class="contents" style="border-spacing:0;font-family:'Helvetica Neue', Helvetica, Arial, sans-serif;color:#555555;width:100%;font-size:13px;text-align:left;" >
														<tr>
															<td class="text" style="padding-top:10px;padding-bottom:10px;padding-right:10px;padding-left:10px;">
																<?php		
																	if ($company_logo)
																	{
															...
															<td>
																...
																<br>
																<?php echo H($this->Location->get_info_for_key('email',isset($override_location_id) ? $override_location_id : FALSE)); ?>
															</td>
														</tr>
													</table>
												</td>
				...
				<tr>
					<td>
						<table width="100%" style="border-spacing:0;font-family:'Helvetica Neue', Helvetica, Arial, sans-serif;color:#555555;font-size:13px;" >
							<tr>
								<td class="inner contents" style="padding-top:5px;padding-bottom:1px;padding-right:10px;padding-left:10px;width:100%;text-align:left;" >
								    <!-- /.row -->
								    <div class="row">
										<!-- to address-->
										...
										<div class="col-md-4 col-sm-4 col-xs-12">
												<?php if(isset($supplier)) { ?>
													<?php echo lang('common_supplier').": ".H($supplier); ?><br />
													<?php if(!empty($account_number)){ ?><?php echo lang('common_ruc_number'); ?> : <?php echo H($account_number); ?><br /><?php } ?>
													<?php if(!empty($supplier_address_1)){ ?><?php echo lang('common_address'); ?> : <?php echo H($supplier_address_1.', '.$supplier_address_2); ?> <?php } ?>
													<?php if (!empty($supplier_city)) { echo ' '.H($supplier_city.', '.$supplier_state.' '.$supplier_zip).'<br />';} ?>
													<?php if (!empty($supplier_country)) { echo ' '.H($supplier_country).'<br />';} ?>			
													<?php if(!empty($supplier_phone)){ ?><?php echo lang('common_phone_number'); ?> : <?php echo H($supplier_phone); ?><br /><?php } ?>
													<?php if(!empty($supplier_email)){ ?><?php echo lang('common_email'); ?> : <?php echo H($supplier_email); ?><br /><?php } ?>
													<?php
													...
													?>
										  			...
										  			<div class="invoice-desc">
																...
																echo '<span>'.lang('common_supplier').' '.($this->Supplier->get_custom_field($custom_field_id,'hide_field_label') ? '' : $this->Supplier->get_custom_field($custom_field_id,'name').':').'</span> '.$format_function($supplier_info->{"custom_field_${custom_field_id}_value"}).'<br />';
																?>
													</div>
															<?php
														}
												...
												<?php } ?>
												<?php if(isset($transfer_to_location)) { ?>
													<span><?php echo lang('receivings_transfer_from').': ' ?></span><?php echo H($transfer_from_location); ?><br />
													<span><?php echo lang('receivings_transfer_to').': ' ?></span><?php echo H($transfer_to_location); ?><br />
												<?php } ?>
										</div>
										<?php } ?>
								    </div>
							...
							<tr>
								<td class="inner no-padding" style="padding-top:10px;padding-bottom:10px;padding-right:10px;padding-left:10px;" >
									<table width="100%" class="items-table" style="border-spacing:0;font-family:'Helvetica Neue', Helvetica, Arial, sans-serif;color:#555555;font-size:13px;padding-top:10px !important;" >
											...
											foreach(array_reverse($cart_items, true) as $line=>$item)
											{
							 				 ...
							 				 elseif ($item->quantity < 0 && $item->name != lang('common_store_account_payment') && $item->name != lang('common_discount') && $item->name != lang('common_refund') && $item->name != lang('common_fee'))
							 				 {
												...
												<tr class="text-center item-row">
													...
													<?php } ?>
													<td align="right" style="padding-right:0;padding-top:10px !important;padding-left:10px;border-width:1px;border-style:solid;border-color:#DCE0E6;border-bottom-width:0px;border-right-width:0px;padding-bottom:10px;" >
										...
										<tr>
											<td colspan="<?php echo $total_columns-1; ?>">
												<div class="row">
										...
										<!---aqui saldo de cuanta de la tienda -->
		...
		<table>
			...
			<tr>
				<td class="one-column" style="padding-top:0;padding-bottom:0;padding-right:0;padding-left:0;" >
					<table width="100%" style="border-spacing:0;font-family:'Helvetica Neue', Helvetica, Arial, sans-serif;color:#555555;font-size:13px;">
						<tr>
							<td class="inner contents" style="padding-top:0px;padding-bottom:0px;padding-right:10px;padding-left:10px;width:100%;text-align:left;">
								<?php
								foreach($receiving_custom_fields_to_display as $custom_field_id)
								{
									if($this->Receiving->get_custom_field($custom_field_id) !== false && $this->Receiving->get_custom_field($custom_field_id) !== false)
									{
											...
											<?php
												...
												{
													echo strtoupper($this->Receiving->get_custom_field($custom_field_id,'name')).':';
												}
												else
												{
						...
						<tr>
							<td class="inner contents" style="padding-top:0px;padding-bottom:0px;padding-right:10px;padding-left:10px;width:100%;text-align:left;">
								<?php echo 'PRECIO: '.letra_modena($exchange_name); ?>
							</td>
						</tr>
						<tr>
							<td class="inner contents" style="padding-top:0px;padding-bottom:10px;padding-right:10px;padding-left:10px;width:100%;text-align:left;" >
								<p style="Margin:0;font-size:13px;Margin-bottom:10px;" >
