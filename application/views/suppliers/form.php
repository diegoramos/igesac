...
<div class="row">
	...
	<?php echo form_open('suppliers/save/'.$person_info->person_id,array('id'=>'supplier_form','class'=>'form-horizontal')); ?>
			...
			<div class="panel-body">
				<div class="row" id="image_load" style="display: none">
					...
					<div class="row">
						<div class="col-md-12">
							<div class="form-group">
								<label class="col-sm-3 col-md-3 col-lg-2 control-label"><font color="green">Buscar en Sunat</font></label>
								<div class="col-sm-9 col-md-9 col-lg-10">
									<div class="input-group">
										<input type="text" id="nruc" name="nruc" class="form-control" placeholder="Ingrese RUC o DNI" pattern="([0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9]|[0-9][0-9][0-9][0-9][0-9][0-9][0-9][0-9])" autofocus>
										<span class="input-group-btn">
										<button class="btn btn-success" id="buscarruc" type="button"><span class="glyphicon glyphicon-search" aria-hidden="true">
										</span> Buscar datos en la sunat</button>
										</span>
									</div>
								</div>
							</div>
...
$("#buscarruc").click(function(e) {
		...
		$.ajax({
			...
			type: "POST",
			dataType: "json",
			url: "<?php echo site_url("suppliers/buscarRuc");?>",
			beforeSend: function(){
		        $('#image_load').show();
		    },
			...
			if(data['success']!="false" && data['success']!=false)
			{
				...
				{
					if (data['type']=='sunat') {
						$("#first_name").val(data['result'].nombre_comercial);
						$("#address_1").val(data['result'].direccion);
						$("#company_name").val(data['result'].razon_social);
						$("#account_number").val(data['result'].ruc);
						$("#last_name").val('');
					}else if(data['type']=='nosunat') {
						$("#company_name").val(data['result'].nombre+" "+data['result'].paterno+" "+data['result'].materno);
						$("#first_name").val(data['result'].nombre);
						$("#last_name").val(data['result'].paterno+" "+data['result'].materno);
						$("#account_number").val(data['result'].dni);
						$("#address_1").val('');
					}
				}
