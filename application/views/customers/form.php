...
$("#buscarruc").click(function(e) {
	...
	$.ajax({
		...
		type: "POST",
		dataType: "json",
		url: "<?php echo site_url("customers/buscarRuc");?>",
		beforeSend: function(){
	        $('#image_load').show();
	    },
		...
		{
			if (data.type=='sunat') {
				$("#first_name").val(data.razonSocial);	
				$("#address_1").val(data.direccion);
				$("#company_name").val(data.razonSocial);
				$("#account_number").val(data.ruc);
				$("#last_name").val('');
			}else if(data.type=='nosunat') {
				$("#first_name").val(data.nombres);
				$("#last_name").val(data.apellidoPaterno+" "+data.apellidoMaterno);
				$("#account_number").val(data.dni);
				$("#address_1").val('');
			}
		}
		
	}).fail(function( jqXHR, textStatus, errorThrown ){
		alert( "No encuentro nada, está correcto el RUC o DNI?:" + textStatus );
		$("#nruc").val('');
		$("#first_name").val('');
		$("#last_name").val('');
		$("#address_1").val('');
