...
<head>
    <title><?php 
		...
		echo !is_on_demo_host() ?  $this->config->item('company').' -- '.lang('common_powered_by').' Bitzua POS' : 'Demo - POS | Software en linea de facil uso' ?></title>
    <meta charset="UTF-8" />
            ...
            else
            {
				...
				$(".checkForUpdate").click(function(event)
				{
					...
					$.getJSON($(this).attr('href'), function(update_available) 
					{
						...
						if(update_available)
						{
							$(".checkForUpdate").parent().html(<?php echo json_encode(lang('common_update_available').' <a href="http://bitzua.com/downloads.php" target="_blank">'.lang('common_download_now').'</a>');?>);
						}
						else
						{
                    ...
                    <div class="version">
                        ...
                        <?php if (isset($trial_on) && $trial_on === true && !isset($trial_over)) { ?>
                           <div class="alert alert-success">
                            ...
                            <a class="btn btn-block btn-success" href="https://bitzua.com/update_billing.php?store_username=<?php echo $cloud_customer_info['username'];?>" target="_blank"><?php echo lang('common_update_billing_info');?></a>
                        <?php } ?>
                        ...
                        <?php if (isset($subscription_cancelled_within_5_days) && $subscription_cancelled_within_5_days === true) { ?>
                            ...
                            <li>
                                <a class="btn btn-block btn-sm btn-success" href="https://bitzua.com/update_billing.php?store_username=<?php echo $cloud_customer_info['username'];?>" target="_blank"><?php echo lang('login_resignup');?></a>
                            </li>
						</ul>
