<?php $this->load->helper('demo'); ?>
	<?php if(($previous_sale_id = $cart->get_previous_receipt_id()) && !$suspended) { ?>
			<div class="alert alert-danger">
				<?php echo lang('sales_editing_sale'); ?> <strong><?php echo $serie;//$this->config->item('sale_prefix').' '.$previous_sale_id; ?></strong>
			</div>
		<?php } ?>
								...
								<td colspan="5">
									<dl class="register-item-extra-details dl-horizontal">
										...
										<?php
											...
											{
												$tier_price = $this->Item->get_tier_price_row($tier_id,$item->item_id);
												...
												if ($tier_price)
												{
													...
													$source_data[] = array('value' => $tier_id, 'text' => $tier_name.' - '.$value);
												}else{
			...
			<div class="sale-buttons">
				...
				<?php if ($mode != 'store_account_payment' && $mode != 'purchase_points') { ?>
					<?php if (!$this->cart->get_previous_receipt_id() || $this->cart->suspended) { ?>
						...
						<?php if ($customer_required_check && $suspended_sale_customer_required_check && !$this->config->item('test_mode')) { ?>
							<div class="btn-group">
...
if(count($exchange_rates))
{
	...
	<div class="amount-block exchange">
		<div class="side-heading">
			<?php echo lang('common_type_money'); ?>
		</div>
...
<?php if(count($cart_items) > 0){ ?> 
		...
		<div class="add-coupon"> 
			...
			<select name="tipo_comprobante"  id="tipo_comprobante" required="required"  class="btn btn-primary">
				<option value="4" <?php if ($type_comprobante==4||$type_comprobante==null) { echo "selected"; } ?>>Nota venta</option>
				<option value="1" <?php if ($type_comprobante==1) { echo "selected"; } ?>>Factura</option>
				<!--<option value="2" <?php //if ($type_comprobante==2) { echo "selected"; } ?> >Recibo Honorario</option>-->
				<option value="3" <?php if ($type_comprobante==3||$type_comprobante==null) { echo "selected"; } ?>>Boleta</option>
			...
			<?php echo form_open("sales/add_payment",array('id'=>'add_payment_form', 'autocomplete'=> 'off')); ?>
				...
				<div class="input-group add-payment-form">
					...
					<span class="input-group-addon">
						...
						<a href="#" class="" id="estimate_sale_button2">Cotizar y Suspender</a>
					<?php }else{ ?>
	 ...
	 $('.xeditable').on('hidden', function(e, editable) {
		...
		bootbox.prompt({
		  title: <?php echo json_encode(lang('sales_please_enter_date_of_birth')); ?>,
			inputType: 'text',
			placeholder: <?php echo json_encode(get_date_format());?>,
		...
		<?php if(!isset($customer)) { ?>	
			...
			$( "#customer" ).autocomplete({
				...
				delay: 150,
		 		...
		 		{
		 			var documento='';
		 			if (ui.item.account_number!='') {
		 				documento=ui.item.account_number;
		 			}
		 			if (documento==null) {
						$("#tipo_comprobante option[value=3]").attr('selected', 'selected');
			 			$.post('<?php echo site_url("sales/set_type_comprobante");?>', {tipo_comprobante: 3});
		 			}else{
		 				var tipo=documento.length;
			 			if (tipo==8) {
			 				$("#tipo_comprobante option[value=3]").attr('selected', 'selected');
			 				$.post('<?php echo site_url("sales/set_type_comprobante");?>', {tipo_comprobante: 3});
			 				
			 			}else if(tipo==11){
			 				$("#tipo_comprobante option[value=1]").attr('selected', 'selected');
			 				$.post('<?php echo site_url("sales/set_type_comprobante");?>', {tipo_comprobante: 1});
			 				
			 			}else{
			 				$("#tipo_comprobante option[value=4]").attr('selected', 'selected');
			 				$.post('<?php echo site_url("sales/set_type_comprobante");?>', {tipo_comprobante: 4});
			 			}
		 			}
      ...
      $("#change_cart_date").on("dp.change", function(e) {
		...
		//Estimate Sale
		$("#estimate_sale_button2").click(function(e)
		{
			e.preventDefault();
			bootbox.confirm(<?php echo json_encode(lang("sales_confirm_suspend_sale")); ?>, function(result)
			{
		...
		//Nuevo cambio
		$('#tipo_comprobante').on('keyup change',tipoDocumento);
		function tipoDocumento(e)
		{
			e.preventDefault();
			$.post('<?php echo site_url("sales/set_type_comprobante");?>', {tipo_comprobante: $(this).val()});
		}
		<?php
...
// Re-usable Functions 
	...
	{
		bootbox.prompt({
			title :<?php echo json_encode(lang("sales_confirm_void_delete")." <br> Escribir el motivo"); ?>,
		    ...
		    callback: function (result) {
			    ...
			    post_data.push({
				 ...
				 post_submit('<?php echo site_url("sales/delete/$sale_id_of_edit_or_suspended_sale"); ?>', 'POST',post_data);
					...
					show_feedback('error', <?php echo json_encode("Este mensaje se enviara a la sunat"); ?>, <?php echo json_encode('Por favor ingresar motivo'); ?>,{timeOut: 30000});
				}
		   }
