...
$sale_rec_info = $this->Sale->get_info($sale_info['sale_id'])->result_array();
if (($sale_rec_info) && ($sale_rec_info['0'])) {
	$currentSaleAccoutingId = $sale_rec_info['0']['accounting_id'];
}
?>
	...
	<div class="panel panel-piluku">
			...
			<div class="panel-body">
				...
				<div class="form-group">	
					<?php echo form_label(lang('sales_receipt_number').':', 'sales_receipt',array('class'=>'col-sm-3 col-md-3 col-lg-2 control-label ')); ?>
					<div class="col-sm-9 col-md-9 col-lg-10 sale-s">
						<?php echo anchor('sales/receipt/'.$sale_info['sale_id'], $serie/*$this->config->item('sale_prefix').' '.$sale_info['sale_id']*/, array('target' => '_blank'));?>
					</div>
				</div>
			...
			{
				if ((!$store_account_payment && !$is_purchase_points && $this->Employee->has_module_action_permission('sales', 'edit_sale', $this->Employee->get_logged_in_employee_info()->person_id)) && (empty($currentSaleAccoutingId))) {
			...
			{ 
				...
				<?php
					...
					?>
				<div class="form-group">	
					<?php echo form_label(lang('common_motivo_anulacion').':', 'motivo_anulacion',array('class'=>'col-sm-3 col-md-3 col-lg-2 control-label ')); ?>
					<div class="col-sm-9 col-md-3 col-lg-3">
						<?php echo form_textarea(array('name'=>'motivo_anulacion','value'=>'','rows'=>'4','cols'=>'23', 'id'=>'motivo_anulacion', 'required'=>'required', 'class'=>'form-control text-area'));?>
					</div>
				</div>
				<div class="form-group">
					<div class="col-md-9 col-md-offset-2">
						...
						echo form_submit(array(
							'name'=>'submit_delete',
							'id'=>'submit_delete',
							'style'=>($anular_sunat)?'display:none':'',
							'value'=>lang('sales_delete_entire_sale'),
							'class'=>' btn btn-danger')
						);
						?>
						<span style="<?php echo ($anular_sunat)?'':'display:none' ?>" class="alert alert-danger">No es posible anular paso su limite de 6 días</span>
					</div>
				</div>
